...
input_msgs = pd.read_csv(f'./results/q3/{protocol}-input.csv').sort_values('timestamp').reset_index()
output_msgs = pd.read_csv(f'./results/q3/{protocol}-output.csv').sort_values('timestamp').reset_index()
...
output_currated_df = output_currated_df[output_currated_df['timestamp_start'] > (30000 + output_currated_df['timestamp_start'][0])].reset_index()
...
for i in range(num_of_buckets):
    latency_buckets[i] = {}
    if i == 0:
        latency_buckets[i]['bound'] = output_currated_df['timestamp_start'][0]
    else:
...
ax.set_title(f"NexMark Q8 - {protocol}")
plt.tight_layout()
