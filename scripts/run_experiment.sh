...
if [[ $query == "q1" || $query == "q12-running" ]]; then
    ...
    python ./reachability_query/reachability-client.py -r "$rate" -p "$scale_factor"
fi
...
if [[ $query == "cyclic" ]]; then
  python ./reachability_query/kafka_input_consumer.py "$saving_dir" "$experiment"
  python ./reachability_query/kafka_output_consumer.py "$saving_dir" "$experiment"
  python ./reachability_query/metrics/calculate_latency.py "$saving_dir" "$experiment"
  #python ./reachability_query/metrics/get_99_percentile.py "$saving_dir" "$experiment"
  python ./reachability_query/metrics/get_99_percentile_output.py "$saving_dir" "$experiment"
else
