...
joined_sorted = joined.sort_values('timestamp_x').reset_index(drop=True)
joined_sorted = (joined_sorted[joined_sorted['timestamp_x'] > (30000 + joined_sorted['timestamp_x'][0])]
...
joined_sorted_out = joined_sorted.sort_values('timestamp_y').reset_index(drop=True)

granularity = 1000  # 1 second (ms) (i.e. bucket size)
num_of_buckets = int((joined_sorted_out['timestamp_y'].iloc[-1]-joined_sorted_out['timestamp_y'][0])/granularity) + 10
print(num_of_buckets)
for i in range(num_of_buckets):
    latency_buckets[i] = {}
    if i == 0:
        latency_buckets[i]['bound'] = joined_sorted_out['timestamp_y'][0]
    else:
        latency_buckets[i]['bound'] = latency_buckets[i-1]['bound'] + granularity
    latency_buckets[i]['items'] = []    

for idx, t in enumerate(joined_sorted_out['timestamp_y']):
